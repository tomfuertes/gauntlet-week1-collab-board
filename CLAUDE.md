...
- **AI:** Cloudflare Agents SDK (`AIChatAgent` DO) + Vercel AI SDK v6 (`streamText`, `generateText`, `tool()`). See `docs/ai-architecture.md` for full request lifecycle.
  - **ChatAgent DO:** One per board (instance name = boardId). WebSocket streaming, server-side chat persistence (DO SQLite). Models: GLM-4.7-flash via Workers AI (default, `WORKERS_AI_MODEL` in wrangler.toml) or Claude Haiku 4.5 (if `ENABLE_ANTHROPIC_API=true` + `ANTHROPIC_API_KEY` secret). Daily budget cap (`DAILY_AI_BUDGET_USD`, default $5) tracked per DO instance (Workers AI only). All AI config in wrangler.toml `[vars]`. `workers-ai-provider` shim in `_getModel()` injects `tool_choice: "auto"` as belt-and-suspenders (GLM calls tools natively). See `docs/notes.md` AI Model Pricing table for cost comparison.
