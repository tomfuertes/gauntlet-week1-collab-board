...
aiRoutes.post("/chat", async (c) => {
  ...
  // Tool definitions with inline callbacks
  ...
  const tools: any[] = [
    {
      ...
      function: async (args: { text: string; x?: number; y?: number; color?: string }) => {
        const id = crypto.randomUUID();
        const obj = {
          id,
          type: "sticky" as const,
          x: args.x ?? 100 + Math.random() * 700,
          y: args.y ?? 100 + Math.random() * 500,
          width: 200,
          height: 200,
          rotation: 0,
    ...
    },
    {
      ...
      function: async (args: { x?: number; y?: number; width?: number; height?: number; fill?: string; stroke?: string }) => {
        const id = crypto.randomUUID();
        const obj = {
          id,
          type: "rect" as const,
    ...
    },
    {
      ...
      function: async (args: { id: string; text?: string; x?: number; y?: number; color?: string; fill?: string }) => {
  ...
  try {
    ...
    const response = await runWithTools(
      ...
      { messages, tools },
      { maxRecursiveToolRuns: 10, verbose: false },
    );

    // runWithTools returns { response: string } after tool loop completes
