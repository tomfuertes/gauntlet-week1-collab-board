...
function ImageRenderer({ src, width, height, aiGlowProps }: { src: string; width: number; height: number; aiGlowProps: Record<string, unknown> }) {
...
export const BoardObjectRenderer = React.memo(function BoardObjectRenderer({
  obj, groupProps = {}, aiGlow = false, interactive = false,
  ...
  if (obj.type === "frame") {
    return (
      <Group {...merged}>
        <Rect width={obj.width} height={obj.height} fill="rgba(99,102,241,0.06)" stroke="#6366f1" strokeWidth={2} dash={[10, 5]} cornerRadius={4} {...glowProps} />
  ...
  if (obj.type === "sticky") {
    return (
      <Group {...merged}>
        <Rect width={obj.width} height={obj.height} fill={obj.props.color || OBJECT_DEFAULTS.sticky.color} cornerRadius={8} shadowBlur={aiGlow ? 12 : 5} shadowColor={aiGlow ? "rgba(99,102,241,0.5)" : "rgba(0,0,0,0.3)"} />
  ...
  if (obj.type === "rect") {
    return (
      <Group {...merged}>
        <Rect width={obj.width} height={obj.height} fill={obj.props.fill || OBJECT_DEFAULTS.rect.fill} stroke={obj.props.stroke || OBJECT_DEFAULTS.rect.stroke} strokeWidth={2} cornerRadius={4} {...glowProps} />
      </Group>
    );
  }
  if (obj.type === "circle") {
    return (
      <Group {...merged}>
        <Ellipse x={obj.width / 2} y={obj.height / 2} radiusX={obj.width / 2} radiusY={obj.height / 2} fill={obj.props.fill || OBJECT_DEFAULTS.circle.fill} stroke={obj.props.stroke || OBJECT_DEFAULTS.circle.stroke} strokeWidth={2} {...glowProps} />
      </Group>
    );
  }
  if (obj.type === "line") {
