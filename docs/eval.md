...
Reports written to `scripts/eval-results/<timestamp>.json` (gitignored, kept on disk for reference, schema `eval-report-v2`).
...
**Baseline:** Keep a known-good report in `scripts/eval-results/` named `baseline-<version>.json` for long-term comparison. Reports are gitignored but kept on disk - use `npm run eval:compare` to diff between runs.
